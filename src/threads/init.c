...
#ifdef USERPROG
  /**
   * Process structure creation for init thread is necessary
   * because init thread is not created by thread_create.
   */
  #ifdef PR_USER
  // memory allocation
  process_create(thread_tid());
  #ifdef PR_FS
  process_current()->dir = dir_open_root();
  #endif
